...
운영 CLI는 기존 세션 폐기, 만료 데이터 정리, 기존 소셜 가입자의 최고 관리자 지정·회수를 제공한다. 사용자 생성이나 토큰 발급은 하지 않는다.
...
신규·기존 사용자는 기본 `tier=free`, `role=user`다. 최고 관리자가 승인한 사용자만 `tier=pro`가 된다. 과금·구독·자동 만료는 없으며 승인을 회수하면 무료로 돌아간다. 프로 등급과 관리자 역할은 독립적이다.

소셜 로그인한 운영자 자신의 `/v1/me` ID로 서버에서 최고 관리자 역할을 지정한다.
...
최고 관리자 토큰으로 승인·회수 API를 호출하며, 변경 사항과 이력을 함께 commit한다. 동일 상태로 반복 요청하면 이력을 중복 생성하지 않는다. 기존 토큰을 그대로 사용해도 다음 요청에서 바뀐 권한을 검사한다. 상세 계약·운영 순서는 [프로 승인 문서](docs/pro-access.md)를 참고한다.

`0003_pro_access`는 기존 계정·소셜 로그인 세션·주차 기록을 유지하면서 기본 무료 등급을 추가한다. 이 마이그레이션만 롤백하면 등급·관리자 역할·변경 이력이 제거된다. 첫 가입자에게 관리자 권한을 자동 부여하지 않는다.
