...
위 검증 기록은 이전 웹형 PoC에 대한 것이다. 현재 앱형 재설계에서는 휴대폰 세로·가로와 태블릿 세로·가로를 브라우저 크기로 먼저 확인한다. 실제 기기나 대상 사용자의 사용성 검증은 아직 수행하지 않았다. 카드 고정·숨기기, 모호한 일정의 추가 질문, 네트워크 실패·오프라인 복구는 후속 검토 범위다.
