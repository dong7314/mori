...
실제 서비스 프론트엔드와 모바일 위젯 구현은 사용자가 담당한다. AI는 사용자가 요청한 범위의 `poc` 화면과 API 계약·예시 데이터를 준비한다. **초기 실제 제품은 React 기반 반응형 웹으로 개발한다.** 모바일 앱은 이후 추가하며 React Native + Expo를 우선 검토한다. 모바일 기술 선택은 아직 확정하지 않았다.
...
| --- | --- | --- |
| 대시보드·일정·채팅·파일 결과 | 초기 제품에서 구현 | 같은 API로 모바일 화면 제공 |
| 음성 입력 | 브라우저 마이크 권한과 지원 형식 확인 | 모바일 녹음 API·권한·중단 처리에 맞게 구현 |
| 작업 승인·거절 화면 | 웹에서 구현 가능 | 앱에서도 같은 승인 API 사용 |
| 푸시 알림 | 지원 브라우저와 설치·권한 조건에 따라 가능 | OS 푸시와 기기 토큰·권한 관리 |
| 예약 알림·알림에서 승인하기 | 웹만으로 모든 OS에서 동일하게 제공한다고 가정하지 않음 | 로컬 알림·알림 액션을 플랫폼별로 구현·검증 |
...
승인은 제품 작업의 실행 승인으로 해석한다. 알림은 승인 요청을 알리는 전달 수단이고, 최종 승인·거절·만료 판단은 서버가 한다. 잠금 화면의 바로 승인과 앱을 열어 내용을 확인한 뒤 승인하는 동작을 작업 영향도에 맞게 구분한다. 마이크·알림 같은 OS 권한 승인은 별도 권한 요청 흐름이다.

### 모바일 기술 제안

React 경험을 활용하면서 위젯·음성·알림·승인 흐름을 모바일에 맞게 구성하려면 React Native + Expo를 우선 검토한다. React Native는 네이티브 기능을 연결하는 한 가지 선택이다. 기존 React 웹 화면의 재사용 비중을 높이는 것이 더 중요하면 Capacitor와 네이티브 플러그인/확장도 비교할 수 있다. [Capacitor 공식 문서](https://capacitorjs.com/docs).

React Native를 채택해도 웹 DOM/CSS 화면이 자동 변환되지는 않는다. React Native는 `View`, `Text` 등 네이티브 컴포넌트를 사용한다. 따라서 공유할 로직과 플랫폼별 화면의 경계를 미리 잡되, 처음부터 모든 UI를 공통화하려고 웹 구현을 복잡하게 만들지 않는다. [React Native 컴포넌트](https://reactnative.dev/docs/intro-react-native-components).

| 공유 후보 | 플랫폼별 구현 |
| --- | --- |
| API 요청/응답 타입과 검증 스키마 | 웹 DOM/CSS, 모바일 화면·내비게이션 |
| 도메인 규칙·날짜 처리·순수 함수 | 녹음·파일 선택·다운로드·공유 |
| 전송·인증 방식을 주입받는 API 클라이언트 | 브라우저 세션/보안과 앱 자격 증명 저장 |
| 플랫폼 의존성을 제거한 상태·데이터 조회 로직 | 마이크·알림·OS 권한, 위젯, 딥링크 |
| 색상·간격 같은 디자인 값 | 플랫폼별 스타일과 접근성 처리 |

공통 코드에 `window`, `document`, `localStorage`를 직접 사용하지 않고 필요한 저장·인증·기기 기능을 인터페이스로 분리한다. 폴더·공유 패키지의 구체적인 구조는 실제 코드가 생기는 시점에 정한다. 웹 세션 방식을 모바일까지 억지로 동일하게 만들 필요는 없다.

### 위젯과 알람의 별도 구현 범위

iOS 위젯은 WidgetKit 확장으로 동작한다. 현재 Expo의 `expo-widgets`는 Expo UI를 이용해 iOS 위젯을 작성하는 경로를 제공하므로 항상 SwiftUI를 직접 작성해야 하는 것은 아니다. 다만 일반 웹/RN 화면을 그대로 사용하는 방식은 아니며, Expo Go 대신 개발 빌드가 필요하다. Android에는 같은 라이브러리의 지원을 가정하지 않고 App Widgets/Jetpack Glance 또는 호환 라이브러리를 따로 검증한다. [Expo Widgets](https://docs.expo.dev/versions/latest/sdk/widgets/), [Android Glance](https://developer.android.com/develop/ui/compose/glance).

일반적인 예약 알림과 ‘정해진 시각에 울리고 사용자가 멈추는 기상 알람’은 요구사항을 구분한다. Expo Notifications는 알림 예약과 승인/거절 같은 알림 액션을 구현하는 후보이다. 기상 알람 수준이면 지원 OS·사용자 권한과 추가 네이티브 연동을 별도로 검토한다. iOS/iPadOS 26의 AlarmKit, Android의 exact alarm이 관련 플랫폼 API이며, 일반 푸시만으로 같은 동작을 보장하지 않는다. [Expo Notifications](https://docs.expo.dev/versions/latest/sdk/notifications/), [Apple AlarmKit 소개](https://developer.apple.com/videos/play/wwdc2025/230/), [Android 알람](https://developer.android.com/develop/background-work/services/alarms).

### 권장 진행 순서

1. React 반응형 웹으로 대시보드·음성 입력·일정·승인 화면과 API를 검증한다.
2. 웹에서 업무 로직과 브라우저 의존 코드를 구분하고, 모바일과 공유할 계약을 유지한다.
3. 모바일 개발을 시작할 때 대상 기기에서 위젯 1개·예약 알림 1개·승인 응답 1개를 먼저 검증해 기술 선택을 확정한다.
4. 모바일 앱을 추가하고 필요한 화면을 구현한다. 웹 서비스와 백엔드는 계속 재사용한다.
