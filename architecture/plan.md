...
```mermaid
flowchart TD
    App[초기 React 반응형 웹 / 후속 모바일 앱] --> API[Mori API: 인증·업무 처리]
    Widget[휴대폰 위젯] --> API
...
React 웹은 초기 실제 제품이다. 이후 모바일 앱도 같은 기록·일정·작업·승인 API를 사용하며, 기기 등록·푸시·위젯·로컬 알림 연동을 추가한다. React Native + Expo는 모바일 후보이며 확정 기술은 아니다. 실행 승인 상태는 서버에 보존해 웹과 앱의 중복 응답을 일관되게 처리하고, 모바일 앱의 백그라운드 상시 실행에 에이전트 작업을 의존시키지 않는다.
